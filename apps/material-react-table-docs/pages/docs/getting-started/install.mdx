...
These are install instructions for the latest version of Material React Table (V4), published as `@mini_7/material-react-table`
...
> `@mini_7/material-react-table` requires **Material UI V9** packages as dependencies in your project.
>
...
<InstallCommand packagesString="@mini_7/material-react-table" />

### Install With Required Peer Dependencies (Recommended)

<InstallCommand />

> You do NOT need to install `@tanstack/react-table`, as it is already an internal dependency of `@mini_7/material-react-table`, and must use an exact version already specified internally.
>
...
If `require()` of the package fails with `ERR_REQUIRE_ESM` or a similar error, you are running the CommonJS build on Node older than 22.12. TanStack Table V9 is ESM-only, so the CommonJS build of `@mini_7/material-react-table` needs Node 22.12 or newer to `require()` ES modules. Upgrade Node or switch to ESM.
